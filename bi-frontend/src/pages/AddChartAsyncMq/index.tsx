...
const AddChartAsyncMq: React.FC = () => {
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      const res = await genChartByAiAsyncMqUsingPost(params, {}, uploadFile);
      if (!res?.data) {
        message.error('分析失败');
      } else {
  ...
  return (
    <div className="add-chart-async-mq">
      <Row gutter={isMobile ? 16 : 24}>
        <Col span={getColSpan()}>
          <Card title="智能分析（异步消息队列）" className="mobile-padding">
        ...
        {/* <Col span={getColSpan()}>
          <Card title="消息队列异步分析说明" className="mobile-padding">
            <Space direction="vertical" size="middle" style={{ width: '100%' }}>
              <Alert 
                type="info" 
                showIcon 
                message="消息队列异步处理" 
                description="使用消息队列进行异步处理，提供更高的可靠性和扩展性。"
              />
              <div>
                <h4>消息队列的优势：</h4>
                <ul>
                  <li>高可靠性：消息持久化，避免数据丢失</li>
                  <li>高并发：支持大量并发请求</li>
                  <li>解耦：前后端完全解耦</li>
                  <li>可扩展：易于水平扩展</li>
                  <li>容错性：自动重试和错误处理</li>
              ...
              <Alert 
                ...
                showIcon 
                message="处理时间" 
                description="由于使用消息队列，处理时间可能比普通异步处理稍长，但更加稳定可靠。"
              />
            </Space>
          </Card>
        </Col> */}
